...
The main challenge is how to **reduce information so that it becomes understandable a for a human**. That is a central
...
personalized medecine, so let's hope you'll enjoy this introductive notebook and that it can help you to address your
...
In a second step, if there is enough time for it, we will try to run a survival analysis so as to give you a general
idea of the challenges of such analyses. 

It is where the current science stands! To go further you'll have to become a researcher in bioinformatics and machine
learning. 
...
```{r, echo=F, fig.cap="Cosinus and sinus plot", fig.height=4, fig.width=8, fig.align="center"}
...
you in the `renv` environment) and the function `LoadcBioportal.R` available in the `lib` folder of the github
repository (if you are curious you can go and check the source code of this function).
...
The function allows you to query various tumor types existing from the TCGA. You can use either the official TCGA
nomenclature (luad = lung adenocarcinoma, gbm = glioblastoma), or the name of the organ of origin (lung = luad & lusc,
brain = gbm & lgg).
...
TcgaData <- LoadcBioportal(Organ="luad|gbm",
                           Genes=CgcGenes,
                           ClinicNeeded=T,
                           MutNeeded=F,
                           RNANeeded=T,
...
fit <- glmnet(
  ...
  y=as.factor(TcgaData$CLINIC$study[trainIndex]),
  family="binomial",
  alpha=0,
  lambda=0,
)
...
```{r}
fitCoefs <- as.matrix(coefficients(fit))
fitCoefs <- fitCoefs[!grepl("(Intercept)", rownames(fitCoefs)),,drop=F]

LuadGenes <- fitCoefs[fitCoefs > 0,,drop=F]
LuadGenes <- LuadGenes[order(LuadGenes, decreasing=T),,drop=F]

GbmGenes <- fitCoefs[fitCoefs < 0,,drop=F]
GbmGenes <- GbmGenes[order(GbmGenes, decreasing=F),,drop=F]
```

```{r, fig.width=8, fig.height=4, fig.cap="Top genes from Logistic regression", fig.align="center"}
nGenesPlot <- 20
xx <- barplot(height=c(LuadGenes[1:nGenesPlot], GbmGenes[1:nGenesPlot]),
              col=c(rep(colorsStudy[["luad"]], nGenesPlot), rep(colorsStudy[["gbm"]], nGenesPlot)),
              cex.names=0.7,
              las=2)

text(xx[1:nGenesPlot]+1, 
     y=-1,
     label=rownames(LuadGenes)[1:nGenesPlot],
     pos=2,
     cex=0.7,
     srt=90)
...
text(xx[(nGenesPlot+1):(2*nGenesPlot)]-0.75,
     y=1,
     label=rownames(GbmGenes)[1:nGenesPlot],
     pos=4,
     cex=0.7,
     srt=90)
```
...
plot(fitCv)
print(paste("You best value of lambda is :", fitCv$lambda.min))
```
...
fitReduced <-glmnet(
  ...
  family="binomial",
  alpha=1,
  lambda=fitCv$lambda.min
)
```

```{r}
fitCoefs <- as.matrix(coefficients(fitReduced))
fitCoefs <- fitCoefs[!grepl("(Intercept)", rownames(fitCoefs)),,drop=F]
...
```{r, fig.width=8, fig.height=4, fig.cap="Top genes from Reduced Logistic regression", fig.align="center"}
nGenesPlot <- 20
xx <- barplot(height=c(LuadGenes[1:nGenesPlot], GbmGenes[1:nGenesPlot]),
              col=c(rep(colorsStudy[["luad"]], nGenesPlot), rep(colorsStudy[["gbm"]], nGenesPlot)),
              cex.names=0.7,
...
```{r}
studyPredictedTrain <- predict(fitReduced, newx=as.matrix(TcgaData$EXP[trainIndex,]), type="class")
studyPredictedTest <- predict(fitReduced, newx=as.matrix(TcgaData$EXP[testIndex,]), type="class")
...
1. Fit a non linear model using the gene expression data of the pancreatic adenocarcinomas and cholangiocarninoma. You
   can use any non-linear model that can perform classification (RandomForest, SVM with non-linear kernel, Neural
   Networks, Xgboost, etc.). You shall follow the same steps as we did here.
  + Train/test split
...
Unfortunatly, there are some NAs and form some patients in TCGA data that will handle by just discarding them. This is
acceptable because there is a low number of such cases. It would not be suitable otherwise.
...
The `survfit` function of the survival package compute the cumulative probability of survival that takes into acount
...
The cumulative probability of survival allow you to plot the Kaplan-Meier curve, which is a classical way to visualize
...
Previously your task was to classify the histologies from expression data. Your new task is to predict survival of
patients, still from the gene expression.

Modeling survival data for various parameters is a historical discipline (the Cox model has been published in 1952).
With the emergence of molecular analysis of cancers it has become a big challenge worldwide. As you will see in this
exemple it is much more complicated than histology classification.

Modeling survival looks like a regression task because you have numeric data to predict. However the way to perform your
regression is adapted to the presence of censored data. The Cox model is using each variable (here gene expression) to
model survival data .
