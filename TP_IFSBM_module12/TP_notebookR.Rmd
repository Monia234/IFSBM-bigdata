...
This is actually one of the current main challenges of personalized medecine, so I'll hope you'll enjoye this Notebook and that it can help you to adress your current and/or future challenges.
...
Then I'll will explain to you a simple modeling strategy for classification, that works very well. Finally, if we have time, I'll introduce to you survival analysis and provide you a glimpse on the limits of such analysis. 
...
print("Welcome at Cancer et génomique : Big Data et modèles prédictifs")
set.seed(1234)
...
```{r}
#urlfile<-"https://raw.githubusercontent.com/DITEP/TPBigDataIFSBM/master/CancerGeneCensusCOSMIC.csv"
...
```{r}
# Install and load usefull packages
...
For example here you can query for various tumor types existing in the TCGA. Be careful, you can use either the official TCGA nomeclature (```luad``` = lung adenocarcinoma, ```gbm``` = glioblastoma), or the name of the organ of origin (```lung``` = luad & lusc, ```brain``` = gbm & lgg).
...
Tis function returns a list called ```TCGAdata``` where each request is stored under the following names: 
...
For continuous data such as RNAseq, simply plotting the distribution is a good first step. See that the LoadcBioportal() function has performed some processing of the RNA data because you have set the argument ```NormalizeRNA = T```.
...
What is the distribution of your RNAseq data?
...
Imagine the red and green dots are very far away from each others and clustered togethers, what would be the name of this problem? 
...
LRfit<-glmnet(as.matrix(TCGAdata$EXP[Train,]),as.factor(TCGAdata$CLINIC$study[Train]),
              family = "binomial", alpha = 0, lambda = 0)
```

### Quizz
On which cohort is it best to evaluate the prediction of your model?
...
### Quizz
What is the name of such matrix representing true values and predictied values?

your answer here: 

What is the [accuracy](https://en.wikipedia.org/wiki/Confusion_matrix) of your current predictions (help in the hyperlink)?
...
### Quizz
Can you say these best 20 gene expressions can determine the tumor type as good as 251?

your answer here: 

# Are there better ways to do so?

Say you want to have a limited list of important genes in your model. It is more or less a regularization (or penalization) procedure: you penalize your model by keeping a limited number of genes.
...
### Quizz
Do you think this model needs important regularization to generalize well, and why (max 2 reasons)?
...
LRfitReduced<-glmnet(as.matrix(TCGAdata$EXP[Train,]),as.factor(TCGAdata$CLINIC$study[Train]),
              family = "binomial", alpha = 1, lambda = LRcv$lambda.min)
...
### Quizz
Has Lasso regularization changed the accuracy of your model?
...
### Quizz
If you go to see a clinician and tell him about your new model, do you think he will want to use it?
...
TCGAdata<-LoadcBioportal(Genes = GENES, Organ = "", 
                         ClinicNeeded = T, MutNeeded = F, 
                         RNANeeded = T, NormalizeRNA = T, 
                         FunctionalAnnot = F, PDF = F, Tests=T)
### END CODE ###
...
# Plot SVD in 2D
res.svd<-svd(TCGAdata$EXP, nu = 2, nv = 2)
plot(res.svd$v[,1],res.svd$v[,2],col=ifelse(TCGAdata$CLINIC$study=="paad",1,2), 
     pch=16,main='SVD per study',
     xlab=paste("dim1 = ",100*round(res.svd$d^2/sum(res.svd$d^2),3)[1],"%",sep = ""),
     ylab=paste("dim2 = ",100*round(res.svd$d^2/sum(res.svd$d^2),3)[2],"%",sep = ""))
legend("bottomright",legend=c("paad","chol"), pch=16, cex=0.8,
       col=1:2)
```
...
Congratulation, you have built a classifier that works reasonably well on a clinically difficult task. Nevertheless there are still some work to do before submitting this model to EMA or FDA for approval and commercialisation... To suggest some perspectives, answer to the following questions:

## Questions:

Do you think your model needs improvment or it is the best shot ever? 

What could be the reasons of some errors of your classifier?

What would you suggest if you want to further improve your model?
...
### Quizz
What is the ```TCGAdata$CLINIC$OS_STATUS``` refering to?
...
```{r}
# proba cumulée de survie : 
# proba conditionelle : on prend en compte les censurés, et on calcule la proba a chaque evenement.
# proba cumulée: produit des probas conditionelles 
survival <- survfit(Surv(TCGAdata$CLINIC$OS_MONTHS, TCGAdata$CLINIC$OS_STATUS)~NULL)
summary(survival)

```

The cumulative probability of survival allow you to plot the Kaplan-Meier curve.
```{r}
# plot survival data
...
# A parametric model survival: the Cox model

```{r}
## COX ###

## effet d'une ou plusieurs variables sur la survie
## fonction de risque en fonctions des differentes covariables
## rapport des risques permet de calculer une variable independante du temps
# le rapport de risque (HR) est ####exp(coef)####
# c'est l'augmentation du risque à chaque mois (1 unité) supplémentaire
# Pr(>|z|) correspond à la p value
...
Concordance index is a classical metric. 1 or 0 are perfect concordances and 0.5 is random.
```{r}
...
The concordance in the training and test sets are relatively close (to confirm) but are both low.

## Quizz bonus
What could be the reasons of poor concordance even on the training set?
