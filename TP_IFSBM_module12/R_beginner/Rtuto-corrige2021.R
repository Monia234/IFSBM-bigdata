...
montableau<-read.csv("Rtuto.csv", header=T)
...
clinical$weight<-as.numeric(clinical$weight)
clinical$bmi<-as.numeric(clinical$bmi)
clinical$height<-as.numeric(clinical$height)
clinical$cigarettes_per_day<-as.numeric(clinical$cigarettes_per_day)
...
table(clinical$age_at_diagnosis>(80*365))
...
clinical$age_arrondi <- round(clinical$age_at_diagnosis/365)
...
boxplot(clinical$weight ~ clinical$tumor_stage)
...
boxplot(log10(clinical$weight) ~ clinical$tumor_stage)

# question 22
# représenter graphiquement le sexe ratio par type de cancer

barplot(table(clinical$gender, clinical $disease), las=2)

# question 23
# représenter graphiquement les proportions relatives de la variable "ethnicity" par type de cancer

barplot(table(clinical $ethnicity, clinical $disease), las=2)

# question 24
# représenter graphiquement l'année du diagnostic en fonction du type de cancer

boxplot (clinical$year_of_diagnosis ~ clinical$disease)

# question 25
# représenter graphiquement le nombre de cigarettes consommées par jour en fonction de l'indice bmi 

plot(clinical$cigarettes_per_day, clinical$bmi, xlim=c(0,10),ylim=c(0,100))

# question 26
# Représeter graphiquement la valeur de l'indice "bmi" en fonction du sexe

boxplot(clinical$bmi ~ clinical$gender, ylim=c(0,100))

# question 27
# Représenter graphiquement la distribution de l'âge au diagnostic en 
# fonction du type de cancer. 

boxplot (clinical$age_at_diagnosis/365 ~ clinical$disease, las=2)
...
clinical[complete.cases(clinical[,c("bmi","cigarettes_per_day")]), ]
table(complete.cases(clinical$bmi), complete.cases(clinical$cigarettes_per_day)) 
sum(complete.cases(clinical$bmi,clinical$cigarettes_per_day))
...
par(mfrow=c(1,2))
hist(clinical[which(clinical$gender == "male"),"height"])
hist(clinical[which(clinical$gender == "female"),"height"])
...
mean(clinical[which(clinical$gender == "male"),"height"], na.rm=T)
mean(clinical[which(clinical$gender == "female"),"height"], na.rm=T)

g1<-clinical[which(clinical$gender == "male"),"height"]
g2<-clinical[which(clinical$gender == "female"),"height"]
x<-t.test(g1,g2)
x$p.value
