...
```{r}
step=2
StepBack<-function(dX,W,cache, step=3){
...
for(i in rev(seq(length(W)))){
  dcache<-StepBack(dX,W,cache,step = i)
  dim(dcache$dW)
...
### run train function
```{r}
history<-train(X,W,B,Y,lr=0.001,iteration=100)
```
